...
def get_booking_percent_by_city():
    ...
    city_data = VillaBooking.objects.values("villa__city__name").annotate(
        city_count=Count("id")
    )

    # Format output with percentage
    result = {
        entry["villa__city__name"]: round(
            (entry["city_count"] / total_bookings) * 100, 2
        )
...
def get_monthly_booking_data():
    ...
    for i in range(12):
        ...
        count = VillaBooking.objects.filter(
...
def dashboard(request):
    ...
    if request.user.is_superuser:
        ...
        total_collection = (
            VillaBooking.objects.filter(status="completed").aggregate(
        ...
        total_collection = (
            VillaBooking.objects.filter(
                villa__user=request.user, status="completed"
...
def list_enquiries(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            meal_option = request.POST.get(
                "meal_option", "without_meal"
            )  # Default to without_meal
            ...
            if not all(
                ...
                if check_in >= check_out:
                    ...
                    messages.success(
                        request, f"Enquiry created successfully for {enquiry.name}!"
                    )
    ...
    # Filter by meal option if provided
    meal_option_filter = request.GET.get("meal_option")
    selected_meal_option = None
    if meal_option_filter:
        selected_meal_option = meal_option_filter
        queryset = queryset.filter(meal_option=meal_option_filter)
    ...
    # Get meal option choices
    meal_option_choices = Enquiry.MEAL_CHOICES
        ...
        try:
            ...
            if not all(
                [
                    ...
                    phone_number,
                    email,
                    check_in_datetime_str,
                    check_out_datetime_str,
                    number_of_people,
                ]
            ):
                messages.error(request, "All fields are required.")
                ...
                check_in_datetime = datetime.strptime(
                    check_in_datetime_str, "%Y-%m-%dT%H:%M"
                )
                check_out_datetime = datetime.strptime(
                    check_out_datetime_str, "%Y-%m-%dT%H:%M"
                )
                ...
                if check_in_datetime >= check_out_datetime:
                    ...
                    messages.error(
                        request, "Check-in date and time cannot be in the past."
                    )
                else:
                    ...
                    messages.success(
                        request,
                        f"Event enquiry created successfully for {event_enquiry.name}!",
                    )
